...
> **Write zone:** ALL commands write to `wiki/` (and `raw/` for outputs/), NEVER to `quartz/content/`. After writes complete, run `bash scripts/sync_wiki.sh` then commit + push to `felipeu28 main`. The architectural rule at the top of this file is the single source of truth.

### /brain-ingest
Process all files in `raw/` that have no `ingested: true` in their frontmatter.
...
3. Find or create the most relevant wiki page(s) in `wiki/` and add a wikilink + fact from this source
4. Update `index.md` Source Inventory + stats if this is a new page
5. Add `ingested: true` and `ingested_at: YYYY-MM-DD` to the source file's frontmatter
6. Append entry to `log.md`
7. Run `bash scripts/sync_wiki.sh && python3 scripts/kb-health.py --fix-index`
8. Commit + push to `felipeu28 main`
...
Answer a question using knowledge stored in `wiki/`.

1. Search `wiki/` for pages relevant to the question
2. Return a cited answer with `[[wikilink]]` references
3. Save the answer to `raw/outputs/YYYY-MM-DD-[slug].md`
4. If the answer reveals a gap in the Brain, create a stub page for that topic in `wiki/`
5. Run `bash scripts/sync_wiki.sh`, commit + push new outputs
...
### /brain-lint
Monthly quality audit. Run after bulk ingestion or monthly. Now consolidated with the `## Related`-block linter — both run from one entry point.
...
4. Find all `wiki/people/*` and `wiki/projects/*` pages missing a `## Related` section (delegated to `pi-workspace/bin/lint-related.sh`)
5. Report everything — don't auto-fix
6. Save report to `raw/outputs/brain-lint-YYYY-MM-DD.md`
7. Run `bash scripts/sync_wiki.sh`, commit + push

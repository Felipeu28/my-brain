---
tags:
  - graph/spoke
status: active
last_updated: 2026-05-03
---
# Connectex

**Type:** client
**Slug:** `connectex`
**Last updated:** 2026-05-03
**Repo(s):** [`Moil-Landingpages/connectex`](https://github.com/Moil-Landingpages/connectex), [`Felipeu28/connectex-website`](https://github.com/Felipeu28/connectex-website) (+ Taiwo's landing-page repo, push pending)
**Primary contact:** [[wiki/people/mark-polanco|Mark Polanco]] (mark@connectex.net)
**Org page:** [[wiki/orgs/connectex]]

---

## Status

Closed Apr 9, 2026 with full upfront invoice (1 down + 3 quarterly over 9 months — fronted in full for Buda EDC reimbursement). Custom landing page + `/CRM` + `/ticketing` + Knowledge Base scope. Walkthrough completed Apr 28; go-live targeted for early next week pending Squarespace credentials handover from Mark. Multi-repo build with Taiwo's landing-page push still outstanding.

## Last 7 days

<!-- AUTO: bin/project-activity.sh updates this section daily. Do not edit by hand. -->
_Updated: 2026-05-03 11:02 (window: 2026-04-26 → 2026-05-03)_

**Mentions in meetings + raw (7):**
- [[wiki/meetings/2026-04-28-mark-polanco-connectex-walkthrough]]
- [[wiki/meetings/2026-04-28-website-update-review-internal]]
- [[raw/email-digest-2026-04-27]]
- [[raw/email-digest-2026-04-28]]
- [[raw/email-digest-2026-04-29]]
- [[raw/email-digest-2026-04-30]]
- [[raw/teams-transcript-mark-polanco-andres-2026-04-28]]

## Recent decisions

- 2026-04-28 — Mark Polanco walkthrough completed; go-live agreed for early next week pending Squarespace login. Source: [[wiki/meetings/2026-04-28-mark-polanco-connectex-walkthrough]]
- 2026-04-09 — Deal closed; full upfront invoice (3-quarter payment) accepted to enable Buda EDC reimbursement timing. Source: [[wiki/moil/pipeline]]

## Open questions

- Squarespace credentials — when does Mark hand them over so Moil can flip the DNS?
- Is the Knowledge Base scope locked? (Was discussed but spec not confirmed in walkthrough notes.)
- Taiwo's landing-page repo push — what's blocking it?
- Once go-live is done, what's the cadence — quarterly check-ins, or active platform engagement?
- Is Connectex a reference customer for the next B2B services pitch?

## Connections

- [[wiki/orgs/connectex]]
- [[wiki/people/mark-polanco]]
- [[wiki/moil/active-projects]]
- [[wiki/moil/customers]]
- [[wiki/moil/pipeline]]
- [[wiki/projects/moil]]

---
name: minds index
description: Index of public AI thought leaders tracked in the Brain
type: index
---

# Minds — Public AI Thought Leaders

**Purpose:** Track the minds whose thinking shapes Andres's world. Distinct from `wiki/people/` (personal network). Each page is a living doc updated during weekly X bookmark sweeps.

## What belongs here
- AI lab CEOs and technical leaders (Dario, Sam, Jensen, Sundar, Mira, etc.)
- Researchers whose public writing shapes direction (Karpathy, Ilya, Fei-Fei)
- Operators whose playbooks matter (founders + tool-builders Andres repeatedly bookmarks)

## What does NOT belong here
- Personal network → `wiki/people/`
- Organizations → `wiki/orgs/`
- Frameworks / mental models → `wiki/concepts/`

## Seeded pages — AI Lab Leaders
- [[wiki/minds/dario-amodei]] — CEO, Anthropic
- [[wiki/minds/sam-altman]] — CEO, OpenAI
- [[wiki/minds/jensen-huang]] — CEO, NVIDIA
- [[wiki/minds/elon-musk]] — xAI / Tesla / SpaceX / X owner
- [[wiki/minds/andrej-karpathy]] — Origin of the Brain's LLM Knowledge Base pattern

## Added 2026-04-12 — X Ecosystem Creators
- [[wiki/minds/nick-spisak]] — @NickSpisak_, non-developer Claude educator (most bookmarked educator)
- [[wiki/minds/corey-ganim]] — @coreyganim, SMB AI monetization (most bookmarked overall — 9+ posts)
- [[wiki/minds/hooeem]] — @hooeem, full-course format AI guides (2.8M views)
- [[wiki/minds/jack-dorsey]] — @jack, "From Hierarchy to Intelligence" + Goose
- [[wiki/minds/farza-mohideen]] — @FarzaTV, Farzapedia + Clicky builder

## Template
When adding a new mind, use this skeleton:

```markdown
---
name: first-last
description: one-line role and why tracked
type: mind
---

# First Last

**Type:** mind
**Last updated:** YYYY-MM-DD
**Why tracked:** one paragraph
**Related:** [[wiki/...]], [[wiki/...]]

## Identity
- Role, background, public voice

## Core beliefs
- 3-6 bullets — the load-bearing ideas Andres should remember

## Why Andres tracks them
- 3-4 numbered reasons tied to Moil or the Brain

## Recent signals
- Dated bullets — updated from weekly bookmark sweeps

## Where to follow
- X handles, blog, podcasts

## Moil Relevance
- How their work affects Moil specifically
```

---
title: "Clients"
tags: [hub, moil, consulting, seed]
---

# Clients — Active Engagements

Client work through Moil-Landingpages and Moil Enterprises.

---

## Active Client Sites (April 2026)

| Client | Repo | Status |
|---|---|---|
| Buda HIVE | `buda-hive` | 🟢 Active |
| Queen Creek | `queen-creek` | 🟢 Active |
| Kyle Buda Spotlight | `KyleBudaSpotlight` | 🟢 Active |
| ReferredLocal | `REFERREDLOCAL` | 🟢 Active |
| Omaha Client | `omaha-client-boost` | 🟢 Active |
| Connectex | `connectex` | 🟢 Active |
| Inna CRM | `Inna-CRM` | 🟡 PR open |

## Core Pages
*No client pages yet — add them as clients are onboarded.*

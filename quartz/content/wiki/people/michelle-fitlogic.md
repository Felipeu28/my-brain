---
tags:
  - graph/spoke
  - person/customer
status: active
last_contact: 2026-05-14
client: "[[wiki/orgs/fitlogic]]"
---
# Michelle (FitLogic Practice Manager)

**Type:** person
**Last updated:** 2026-05-14
**Source:** [[raw/teams-transcript-CRM-GOOGLE-Setup-with-Megan-2026-04-23]], [[raw/teams-transcript-megan-moil-crm-test-and-delivery-2026-04-29]], [[raw/teams-transcript-megan-andres-2026-05-07]], [[raw/teams-transcript-megan-andres-2026-05-14]]
**Related:** [[wiki/people/megan-miller]], [[wiki/orgs/fitlogic]], [[wiki/people/daniel-mann]], [[wiki/meetings/2026-04-23-megan-crm-google-setup]], [[wiki/meetings/2026-04-29-megan-fitlogic-crm-delivery]], [[wiki/meetings/2026-05-07-megan-andres-fitlogic-crm-handoff]], [[wiki/meetings/2026-05-14-megan-taiwo-fitlogic-2fa-godaddy-session]], [[wiki/concepts/moil360]]

---

**Title:** Practice Manager — FitLogic Functional Medicine
**Tenure:** Started ~mid-April 2026 (~2 weeks as of Apr 29)
**Relationship:** Megan Miller's right-hand operator; the day-to-day Moil 360 driver at FitLogic

> Last name not yet captured in transcripts. Update once known.

---

## Profile

Michelle is FitLogic's clinic practice manager. She joined the practice ~mid-April 2026, so she's still in early ramp on Megan's clinical operations (Charm EHR, Keap CRM, Square booking, patient flow). She handles inbound calls, pharmacy coordination, and patient-facing comms for Megan.

She **is the de facto day-to-day operator of Moil 360 at FitLogic** — Megan named her in the Apr 29 delivery walkthrough as the person who will run image generation, email campaigns, and posting. Megan brings the clinical authority and brand voice; Michelle drives the platform.

## Why she matters to Moil

Michelle is the **first concrete instance of the "owner trains a manager" delivery pattern** for a Moil 360 customer. The implications:

1. **Onboarding docs/videos should target the manager, not the owner.** Megan signs the cheque; Michelle will spend 4 hours/week in the platform.
2. **Tone calibration runs through her** — she was the one who pushed back on *"I wanted to share…"* and articulated the reader-centered hook framing. *"We need an attention getter to start it off … like a thought-provoking question to get them to read the rest of the email."*
3. **She's a referral path.** She knows [[wiki/people/daniel-mann|Daniel D. Mann]] ("Firefly") socially — first surfaced Apr 23 when she casually mentioned *"I'm supposed to say hello to Andres from Firefly."* This is a second-degree warm loop back through Moil's #1 referral partner.

## Apr 23 — Google Cloud OAuth setup call (cameo)

Source: [[wiki/meetings/2026-04-23-megan-crm-google-setup]]

Joined briefly near the end. Notable items:
- Shared the **`hello@fitlogicfunctionalmedicine.com` Gmail password** with Megan/Andres — same one she already uses
- Mentioned the Daniel Mann ("Firefly") connection and confirmed Daniel says positive things about Andres back: *"smart guy, very well connected"*

## Apr 29 — CRM Test & Delivery (full walkthrough)

Source: [[wiki/meetings/2026-04-29-megan-fitlogic-crm-delivery]]

Michelle joined ~2/3 through the call (after handling a Buda Drug Store medication-refill phone call live). From the moment she joined, she became the most useful copy reviewer in the room:

- **Reframed the email opening line** from *"I wanted to share a quick thought"* → suggested *"You may be wondering…"* / *"Are you still struggling with…"* — Megan immediately said *"a hook, yes, we need the hook"* and the team accepted it.
- **Flagged closed-captioning as an accessibility requirement** — *"People with hearing disabilities comment that a lot. I see that a lot that they need to have that."* First time accessibility surfaced as Moil 360 customer feedback. She volunteered to ask her Austin business friend (who solved this on Instagram) for the workaround.
- **Drove the live test post** — copied the AI-generated post, downloaded the video, switched the Facebook account to the FitLogic page, pasted the caption, attached the video. (Did not publish — *"I'm not going to post it, but I just wanted to go through the process."*)
- **Suggested the "IC-only" tag** for patients who came in for an initial consultation but never converted — became part of the agreed lead-segmentation taxonomy.

## May 7, 2026 — Cross-mentioned in Megan × Andres CRM handoff (not present)

Source: [[wiki/meetings/2026-05-07-megan-andres-fitlogic-crm-handoff]]

Megan reported Michelle is **excelling at the relational side** — one new patient asked Michelle to be physically present in her appointment because she had no one else to bring. Megan flagged this as a strong organic-fit signal for Michelle's role.

Operationally, **next-week's in-person CRM walkthrough explicitly targets Michelle as the primary trainee** (Andres scheduled the visit with Megan + Michelle together). The "owner trains a manager" pattern continues — Megan starts the CRM tomorrow with low-volume sends + the test-contact flag specifically *"while Michelle learns the system."*

## Connections

- **[[wiki/people/megan-miller|Megan Miller]]** — boss; Megan trusts her copy judgment more than her own (*"she's better at putting my thoughts into words"*)
- **[[wiki/people/daniel-mann|Daniel D. Mann]]** — knows him socially as "Firefly"; Michelle relays warm regards from him to Andres

## May 14, 2026 — In-person walkthrough locked for Wed May 20 (cross-referenced)

Source: [[wiki/meetings/2026-05-14-megan-taiwo-fitlogic-2fa-godaddy-session]]

Michelle not present on the May 14 working session, but the **Wed 2026-05-20 10:00–11:00 AM CT in-person walkthrough at FitLogic** with Andres + Megan + Michelle is now locked. Megan + Michelle's Mon/Tue (May 18–19) homework: play with the new Outlook-on-`myfitlogic.com` system, try to break it, surface bugs before the Wednesday session. Continues the "owner trains a manager" pattern — Michelle is the primary trainee on the in-person side.

## Open threads

- Last name not captured — update on next call
- Ask Megan to confirm Michelle's email address for direct comms (currently routes through Megan or `hello@fitlogicfunctionalmedicine.com`)
- Will become primary day-to-day Moil 360 contact at FitLogic; future product-feedback loop should include her, not just Megan

<!-- AUTO-ACTIVITY:start -->
## Recent Activity (auto)

*Auto-generated by entity-graph-builder · last refreshed 2026-05-14. Entries capped at 12, pruned at 30 days. Do not edit inside the markers.*

- **2026-05-14** · `teams` · will test new system with Megan before Wednesday review
<!-- AUTO-ACTIVITY:end -->
